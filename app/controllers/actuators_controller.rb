class ActuatorsController < ApplicationController
  ...
  def index
    ...
    @resful_con = RestfulConnection.new('SBS-UDC-1')
  end
  ...
  def change_status
    ...
    conn = MqttConnection.new
    conn.connect
    value = params[:state]  == 'true' ? "ON" : "OFF"
    conn.publish('SBS-UDC-1', @actuator.name, value )
    conn.close
  ...
  private
    ...
    def actuator_params
      params.require(:actuator).permit(:name, :status, :description, :location)
    end
end
